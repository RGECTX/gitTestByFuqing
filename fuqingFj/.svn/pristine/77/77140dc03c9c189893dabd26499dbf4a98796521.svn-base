...
layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxDagl/search?charset=utf-8&loginCode=" + adminInfo.loginCode;
    var auxDaglTable = table.render({
        elem: '#test',
        url: url,
        method: 'POST',
    ...
    table.on('toolbar(test)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case 'del':
                ...
                layer.confirm('确定删除选中的档案管理信息吗', function (index) {
    ...
    table.on('tool(test)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            getAttachmentList("AuxDagl", data.daId);
            form.val("edit", data);
        ...
        if (obj.event === 'view') {
            getAttachmentListView("AuxDagl", data.daId);
            form.val("view", data);
            layer.open({
                type: 1,
                title: '查看档案管理信息',
    ...
    function addAuxDagl(data) {
        var layerIndex = layer.load();
        $.ajax({
            type: "POST",
            url: servicePath + "/auxDagl/add?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                    ...
                    if ($("#add_uploadList").find("tr").length == 0) {
                        showTicket("保存成功！", 1000, function () {
                            ...
                            if (iframeWindow != undefined) {
                                ...
                                top.topTabDelete("auxiliary/addArchive.html?v=1.60");
                            }
                        ...
                        addUpload.reload({
                            url: servicePath + "/file/upload?charset=utf-8&instanceType=AuxDagl&instanceId=" + data.auxDaglObj.daId + "&loginCode=" + adminInfo.loginCode
                        });
                        addUpload.upload();
                        showTicket("保存成功！", 1000, function () {
                            var iframeWindow = top.getIframeWindow(iframeName);
