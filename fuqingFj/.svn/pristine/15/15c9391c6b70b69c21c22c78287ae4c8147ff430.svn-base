layui.use(['tree', 'util','element','form','table','layer'], function(){
  ...
  var innerMessageTable=table.render({
    ...
    ,where: { "kwFields": 11, "keyword": "","recipient":adminInfo.userId,"msgState":2}
	,limit:10
    ...
    ,cols: [[
      ...
      ,{field:'senderName', title:'发件人',templet: function(d){
  ...
  table.on('tool(innerMessageList)', function(obj){
	...
	//查看消息
	if(obj.event === 'view'){
		getAttachmentListForView("AmInnerMessage",data.innerMessageId);
  ...
  form.on('submit(searchBtn)', function(data){
	...
	//获取附件列表显示于查看
	function getAttachmentListForView(instanceType,instanceId) {
		var layerIndex = layer.load();
		$.ajax({
			type: "GET",
