layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxDagl/rytj?charset=utf-8&loginCode=" + adminInfo.loginCode;
    var auxRytjTable = table.render({
        elem: '#test',
        url: url,
        method: 'POST',
                    ...
                    if ($("#edit_uploadList").find("tr[id]").length == 0) {
                        showTicket("保存成功！", 1000, function () {
                            ...
                            if (iframeWindow != undefined) {
                                iframeWindow.auxRytjTable.reload();
