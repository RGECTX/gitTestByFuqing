...
layui.use(['form', 'table', "upload", 'laydate'], function() {
	...
	function search(formData) {
		var url = servicePath + "/auxFjbzfp/searchTab?charset=utf-8&loginCode=" + adminInfo.loginCode;
		...
		$.ajax({
			...
			success: function(data) {
				/* var auxFjbzfpSearchCriteriaList = data.auxFjbzfpSearchCriteriaList;
				console.log(auxFjbzfpSearchCriteriaList); */
				...
				if (data.errCode == 0) {
					...
					$("#tbodyId").append(
						...
						'<th colspan="2">联防辅警</th>' +
						/*'<th rowspan="2">状态</th>' +*/
						'<th colspan="2">专项辅警</th>' +
						'<th colspan="2">普通辅警</th>' +
						/*'<th rowspan="2">年度</th>' +
						'<th rowspan="2">月度</th>' +
					...
					var thStr = '<tr>' +
						'<td class="lock">现有<br>人数</td>' +
						'<td class="lock">编制<br>人数</td>' +
						'<td class="lock">现有<br>人数</td>' +
						'<td class="lock">编制<br>人数</td>' +
						'<td class="lock">现有<br>人数</td>' +
						'<td class="lock">编制<br>人数</td>' ;

					/*for (var p = 0; p < fjbzfpList.length; p++) {
						thStr = thStr + '<td class="lock">' + '</td>';
					}*/

					thStr = thStr + '</tr>';
					$("#tbodyId").append(thStr);
					...
					var jfly2Sum = 0;
					var jfly4Sum = 0;
					$.each(data.searchCriterias, function(index, searchCriterias) {
						$("#tbodyId").append('<tr>' +
							'<td>' + (index + 1) + '</td>' +
							'<td>' + (searchCriterias.orgName || '') + '</td>' +
							'<td>' + (searchCriterias.existingPeople1 || 0) + '</td>' +
							...
							'<td>' + (searchCriterias.formationPeople1 || 0) + '</td>' +
							'<td>' + (searchCriterias.existingPeople2 ||0) + '</td>' +
							'<td>' + (searchCriterias.formationPeople2 || 0) + '</td>' +
							'<td>' + (searchCriterias.existingPeople4 || 0) + '</td>' +
							'<td>' + (searchCriterias.formationPeople4 || 0) + '</td>' +
