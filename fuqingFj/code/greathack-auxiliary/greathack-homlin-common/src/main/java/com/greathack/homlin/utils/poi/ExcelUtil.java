...
import com.greathack.homlin.core.text.Convert;
import com.greathack.homlin.exception.BusinessException;
import com.greathack.homlin.utils.reflect.ReflectUtils;
            ...
            if (cell != null)
            {
                if (cell.getCellTypeEnum() == CellType.NUMERIC)
                {
                    ...
                    if (HSSFDateUtil.isCellDateFormatted(cell))
                    {
                        ...
                        val= com.greathack.homlin.utils.DateUtils.formatDate((Date)val,"yyyy-MM-dd");
                        //val = cell.getStringCellValue();//不转化日期格式直接在service层进行日期格式判断，2020-02-14
                    }
                    else
                    {
                        if ((Double) val % 1 > 0)
