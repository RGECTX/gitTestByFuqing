	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
			...
			*/
			...
			"allowFile":function(gets,obj,curform,datatype){				
				if(gets==""){
				...
				return checkFile(obj.get(0),obj.attr("allowType"),obj.attr("maxSizeByKB"));
				//判断选择上传的文件是否符合条件
				//inputFile:文件域对象
				//allowType:允许上传的扩展名,格式：.jpg|.gif|.png
				//允许上传的文件大小，以KB为单位
				function checkFile(inputFile,allowType,maxSizeByKB){
					
					index=inputFile.value.lastIndexOf(".");
					ext=inputFile.value.substring(index,inputFile.value.length);
					if(allowType.indexOf(ext)==-1){
					   ...
					   if (isIE && !inputFile.files) {
						  ...
						  file = fileSystem.GetFile (inputFile.value);
						  fileSize = file.Size;
					   } else {
						  fileSize = inputFile.files[0].size;
					   }
					   if(fileSize / 1024>maxSizeByKB){
						   return obj.attr("errormsg");
					   }else{
						   return true;
					   }
