layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxKqtj/search?charset=utf-8&loginCode=" + adminInfo.loginCode;
    var auxKqlrTable = table.render({
        elem: '#test',
        url: url,
        method: 'POST',
        ...
        cols: [
            [{
                ...
                field: 'isWorkFullHours',
                title: '是否满勤',
                align: "center",
                width: 100,
                templet: function (d) {
                    /*return getDictDataLabel("AUX_ISWORKFULLHOURS", d.isWorkFullHours)*/
                    if (d.isWorkFullHours == '1') {
                        return '<font color=green >满勤</font>'
                    } else if (d.isWorkFullHours == '2') {
                        return '<font color=red >未满勤</font>'
                    }else{
                        return '<font color=red >未满勤</font>'
                    }
                }
        ...
        response: {
            ...
            dataName: 'auxKqtjList' //规定数据列表的字段名称，默认：data
        },
        request: {
        ...
        /*        formData.reportId = adminInfo.amUnitId;
            ...
            case 'del':
                ...
                layer.confirm('确定删除选中的考勤录入信息吗', function (index) {
                    ...
                    var url = servicePath + "/auxKqtj/delete?charset=utf-8&loginCode=" + adminInfo.loginCode + "&ids=" + ids;
                    $.ajax({
                        type: "POST",
                        url: url,
                        contentType: "application/json; charset=utf-8",
    ...
    table.on('tool(test)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            getAttachmentList("AuxKqlr", data.kqtjId);
            form.val("edit", data);
            layer.open({
                type: 1,
                title: '修改考勤录入信息',
        ...
        if (obj.event === 'view') {
            getAttachmentListView("AuxKqlr", data.kqtjId);
            form.val("view", data);
            layer.open({
                type: 1,
                title: '查看考勤录入信息',
    ...
    function addAuxKqlr(data) {
        var layerIndex = layer.load();
        $.ajax({
            type: "POST",
            url: servicePath + "/auxKqtj/add?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    if ($("#add_uploadList").find("tr").length == 0) {
                        ...
                        addUpload.reload({
                            url: servicePath + "/file/upload?charset=utf-8&instanceType=AuxKqlr&instanceId=" + data.auxKqtjObj.kqtjId + "&loginCode=" + adminInfo.loginCode
                        });
                        addUpload.upload();
                    }
                } else if (data.errCode == 200061 || data.errCode == 200062) {
    ...
    function editAuxKqlr(data) {
        ...
        $.ajax({
            type: "POST",
            url: servicePath + "/auxKqtj/update?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    if ($("#edit_uploadList").find("tr[id]").length == 0) {
                        ...
                        editUpload.reload({
                            url: servicePath + "/file/upload?charset=utf-8&instanceType=AuxKqlr&instanceId=" + formData.kqtjId + "&loginCode=" + adminInfo.loginCode
                        });
                        editUpload.upload();
                    }
                } else if (data.errCode == 200061 || data.errCode == 200062) {
