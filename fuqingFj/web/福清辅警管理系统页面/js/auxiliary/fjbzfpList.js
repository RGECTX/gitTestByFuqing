layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxFjbzfp/search?charset=utf-8&loginCode=" + adminInfo.loginCode;
    var auxFjbzfpTable = table.render({
        elem: '#test',
        url: url,
        method: 'POST',
        ...
        response: {
            ...
            dataName: 'auxFjbzfpList' //规定数据列表的字段名称，默认：data
        },
        request: {
    ...
    table.on('toolbar(test)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case 'del':
                ...
                layer.confirm('确定删除选中的辅警编制分配情况信息吗', function (index) {
                    var layerIndex = layer.load();
                    var url = servicePath + "/auxFjbzfp/delete?charset=utf-8&loginCode=" + adminInfo.loginCode + "&ids=" + ids;
                    $.ajax({
                        type: "POST",
                        url: url,
                        contentType: "application/json; charset=utf-8",
    ...
    table.on('tool(test)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            getAttachmentList("AuxFjbzfp", data.fjbzfpId);
            form.val("edit", data);
            //通过ID的方法getOrgName获取到目标值
            /*$("#orgId").val(getOrgName(data.orgId));*/
            layer.open({
                type: 1,
                title: '修改辅警编制分配情况信息',
                area: ['800px', '620px'],
        ...
        if (obj.event === 'view') {
            getAttachmentListView("AuxFjbzfp", data.fjbzfpId);
            form.val("view", data);
            layer.open({
                type: 1,
                title: '查看辅警编制分配情况信息',
    ...
    function addAuxFjbzfp(data) {
        var layerIndex = layer.load();
        $.ajax({
            type: "POST",
            url: servicePath + "/auxFjbzfp/add?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
    ...
    function editAuxDagl(data) {
        ...
        $.ajax({
            type: "POST",
            url: servicePath + "/auxFjbzfp/update?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
    ...
    //搜索访问民警信息
    function searchAuxFjbzfp() {
        var formData = form.val("searchForm");
        var data = {
            orgName: formData.orgName,
            orgId: formData.orgId,
            fjType: formData.fjType
        }
        auxFjbzfpTable.reload({
            where: data
        });
    ...
    var addUpload = fileUpload($("#add_uploadList"), '#add_selectFileBtn', "AuxDagl", 1);
...
//关闭当前tab页面
function closeTab() {
    top.topTabDelete("auxiliary/addArchive.html?v=1.74");
}
