layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxGsgl/search?charset=utf-8&loginCode=" + adminInfo.loginCode;
    auxGsglTable = table.render({
        elem: '#test',
        url: url,
        method: 'POST',
        ...
        response: {
            ...
            dataName: 'auxGsglList' //规定数据列表的字段名称，默认：data
        },
        request: {
        ...
        /*        formData.reportId = adminInfo.amUnitId;
            ...
            case 'del':
                ...
                layer.confirm('确定删除选中的工资信息吗', function (index) {
                    ...
                    var url = servicePath + "/auxGsgl/delete?charset=utf-8&loginCode=" + adminInfo.loginCode + "&ids=" + ids;
                    $.ajax({
                        type: "POST",
                        url: url,
                        contentType: "application/json; charset=utf-8",
    ...
    table.on('tool(test)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            getAttachmentList("AuxGsgl", data.gsglId);
            form.val("edit", data);
            layer.open({
                type: 1,
                title: '修改工伤录入信息',
        ...
        if (obj.event === 'view') {
            getAttachmentListView("AuxGsgl", data.gsglId);
            form.val("view", data);
            layer.open({
                type: 1,
                title: '查看工伤登记信息',
    ...
    function addAuxGssb(data) {
        var layerIndex = layer.load();
        $.ajax({
            type: "POST",
            url: servicePath + "/auxGsgl/add?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    if ($("#add_uploadList").find("tr").length == 0) {
                        ...
                        addUpload.reload({
                            url: servicePath + "/file/upload?charset=utf-8&instanceType=AuxGsgl&instanceId=" + data.auxGsglObj.gsglId + "&loginCode=" + adminInfo.loginCode
                        });
                        addUpload.upload();
                    }
                } else if (data.errCode == 200061 || data.errCode == 200062) {
    ...
    function gssb(data) {
        ...
        $.ajax({
            type: "POST",
            url: servicePath + "/auxGsgl/updateState?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    if ($("#edit_uploadList").find("tr[id]").length == 0) {
                        showTicket("提交成功！", 1000, function () {
                            ...
                            var iframeWindow = top.getIframeWindow("auxiliary/gsshList.html?v=1.60");
                            if (iframeWindow != undefined) {
                                iframeWindow.auxGsglTable.reload();
                                top.topTabDelete("auxiliary/gssbList.html?v=1.60");
                            }
                        });
                    } else {
                        editUpload.reload({
    ...
    function editAuxGssb(data) {
        var formData = data;
        var layerIndex = layer.load();
        $.ajax({
            type: "POST",
            url: servicePath + "/auxGsgl/update?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
    ...
    form.on('submit(add)', function (data) {
        addAuxGssb(form.val("add"));
        return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
    });

    //修改个人政治学习申请信息“保存”按钮触发
    form.on('submit(edit)', function (data) {
        editAuxGssb(form.val("edit"));
        return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
    });


    //搜索访问民警信息
    function searchAuxGsgl() {
    ...
    var addUpload = fileUpload($("#add_uploadList"), '#add_selectFileBtn', "AuxGsgl", 1);

    var editUpload = fileUpload($("#edit_uploadList"), '#edit_selectFileBtn', "AuxGsgl", 1);
