layui.use(['form', 'table', 'laydate'], function () {
    ...
    function getObject() {
        ...
        var bzglId = getParameter('bzglId');

        getAttachmentListView("AuxBzgl", bzglId);

        var url = servicePath + "/auxBzgl/getObject?charset=utf-8&loginCode=" + adminInfo.loginCode + "&bzglId=" + bzglId;
        $.ajax({
            type: "POST",
            url: url,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({"bzglId": bzglId}),
            dataType: "json",
            success: function (data) {
                form.val("addAdmin", data.auxBzgl);
                form.render('select');
            }
        })
    }
            ...
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    showTicket("本环节完成，进入下一个环节！", 2000, function () {
                        ...
                        var iframeWindow = top.getIframeWindow("auxiliary/dclBzglList.html?v=1.60");
...
function closeTab() {
    var bzglId = getParameter('bzglId');
    top.topTabDelete("auxiliary/viewBzgl.html?v=1.74&bzglId=" + bzglId);
}
