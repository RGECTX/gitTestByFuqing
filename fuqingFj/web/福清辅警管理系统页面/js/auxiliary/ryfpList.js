layui.use(['form', 'table', "upload", "laydate"], function () {
    ...
    var url = servicePath + "/auxZljd/searchAprove?charset=utf-8&loginCode=" + adminInfo.loginCode;
    ...
    table.on('tool(test)', function (obj) {
        ...
        /*if (obj.event === 'view') {
            ...
            /!*top.topTabAdd("查看招录建档流程",
                '<div class="layui-body"><iframe src="auxiliary/viewZljd.html?v=1.74&zlId=' + data.zlId + '&processId=' + processId + '" frameborder="0" class="layadmin-iframe"></iframe></div>',
                'fuding_bajf/viewJfsq.html?v=1.74&zlId=' + data.zlId)*!/

            top.topTabAdd("查看招录建档流程",
                '<div class="layui-body"><iframe src="auxiliary/viewRyfp.html?v=1.74&zlId=' + data.zlId + '&processId=' + processId + '" frameborder="0" class="layadmin-iframe"></iframe></div>',
                'fuding_bajf/viewJfsq.html?v=1.74&zlId=' + data.zlId)
    ...
    function editAuxZljd(data) {
        ...
        $.ajax({
            type: "POST",
            url: servicePath + "/auxZljd/updateFjlb?charset=utf-8&loginCode=" + adminInfo.loginCode,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: "json",
            success: function (data) {
                ...
                if (data.errCode == 0) {
                    if ($("#edit_uploadList").find("tr[id]").length == 0) {
                        showTicket("保存成功！", 1000, function () {
                            ...
                            var iframeWindow = top.getIframeWindow("auxiliary/dwjsList.html?v=1.60");
                            if (iframeWindow != undefined) {
                                iframeWindow.auxZljdTable.reload();
                                top.topTabDelete("auxiliary/ryfpList.html?v=1.60");
                            }
