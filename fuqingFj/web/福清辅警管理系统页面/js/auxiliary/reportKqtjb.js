...
layui.use(['form', 'table', "upload", 'laydate'], function() {
	...
	function search(formData) {
		var url = servicePath + "/auxKqtj/getFjqktj?charset=utf-8&loginCode=" + adminInfo.loginCode;
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.errCode == 0) {
					...
					var jfly2Sum = 0;
					var jfly4Sum = 0;
					$.each(data.amAttendanceSearchCriteriaList, function(index, amAttendanceSearchCriteria) {
						$("#tbodyId").append('<tr>' +
							'<td>' + (index + 1) + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.orgName || '') + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.xm || 0) + '</td>' +
							/*'<td>' + (amAttendanceSearchCriteria.isWorkFullHours || 0) + '</td>' +*/
							'<td>' + (amAttendanceSearchCriteria.xb == 1 ? '男' : '女') + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.idcard || 0) + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.state == 1 ? '<font color=blue >在岗</font>' : '<font color=red >离职</font>') + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.isWorkFullHours == 2 ? '是' : '否') + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.nd || 0) + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.yd || 0) + '</td>' +
							'<td>' + (amAttendanceSearchCriteria.qqTs || 0) + '</td>' +
						/*	'<td>' + (amAttendanceSearchCriteria.age3 || 0) + '</td>' +
							...
							'<td>' + (amAttendanceSearchCriteria.jfly4 || 0) + '</td>' +*/
							'</tr>')
						qqts += Number(amAttendanceSearchCriteria.qqTs);

						cntSum += Number(amAttendanceSearchCriteria.xm);
						sex2Sum += Number(amAttendanceSearchCriteria.sex2);
						age1Sum += Number(amAttendanceSearchCriteria.age1);
						age2Sum += Number(amAttendanceSearchCriteria.age2);
						age3Sum += Number(amAttendanceSearchCriteria.age3);
						age4Sum += Number(amAttendanceSearchCriteria.age4);
						xl896Sum += Number(amAttendanceSearchCriteria.xl896);
						xl112Sum += Number(amAttendanceSearchCriteria.xl112);
						xl12Sum += Number(amAttendanceSearchCriteria.xl12);
						xl3Sum += Number(amAttendanceSearchCriteria.xl3);
						xl1024Sum += Number(amAttendanceSearchCriteria.xl1024);
						zzmm3Sum += Number(amAttendanceSearchCriteria.zzmm3);
						zzmm4Sum += Number(amAttendanceSearchCriteria.zzmm4);
						zzmm8Sum += Number(amAttendanceSearchCriteria.zzmm8);
						workAge1Sum += Number(amAttendanceSearchCriteria.workAge1);
						workAge2Sum += Number(amAttendanceSearchCriteria.workAge2);
						workAge3Sum += Number(amAttendanceSearchCriteria.workAge3);
						gwlb001Sum += Number(amAttendanceSearchCriteria.gwlb001);
						gwlb002Sum += Number(amAttendanceSearchCriteria.gwlb002);
						zpfs1Sum += Number(amAttendanceSearchCriteria.zpfs1);
						zpfs2Sum += Number(amAttendanceSearchCriteria.zpfs2);
						jfly1Sum += Number(amAttendanceSearchCriteria.jfly1);
						jfly2Sum += Number(amAttendanceSearchCriteria.jfly2);
						jfly4Sum += Number(amAttendanceSearchCriteria.jfly4);
					})
					$("#tbodyId").append('<tr>' +
						'<td colspan="2">合计</td>' +
						'<td>' + " " + '</td>' +
