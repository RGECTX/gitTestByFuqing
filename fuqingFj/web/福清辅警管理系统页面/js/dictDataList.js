layui.use(['form', 'table', 'jquery', 'laydate'], function() {
	...
	var dictId=getParameter('dictId');
	var dictCode=getParameter('dictCode');
	//alert(dictCode)
	$("#dictCode").val(dictCode);
	var dictTable = table.render({
		elem: '#dictList',
		url: servicePath + "/dictData/getList?charset=utf-8&loginCode=" + adminInfo.loginCode+"&dictId="+dictId,
		method: 'POST',
		...
		response: {
			...
			dataName: 'dictDataList' //规定数据列表的字段名称，默认：data
		},
		request: {
	...
	table.on('tool(dictList)', function(obj) {
		...
		if (obj.event === 'del') {
			layer.confirm('确定删除该字典数据吗？', function(index) {
				obj.del();
				del(data.dictDataId);
				layer.close(index);
			});
	...
	function add(data) {
		...
		$.ajax({
			type: "POST",
			url: servicePath + "/dictData/add?charset=utf-8&loginCode=" + adminInfo.loginCode,
			contentType: "application/json; charset=utf-8",
	...
	function edit(data) {
		...
		$.ajax({
			type: "POST",
			url: servicePath + "/dictData/update?charset=utf-8&loginCode=" + adminInfo.loginCode,
			contentType: "application/json; charset=utf-8",
			data: JSON.stringify(data),
			dataType: "json",
			success: function(data) {
	...
	//删除字典类型
	function del(dictDataId) {
		var layerIndex = layer.load();
		$.ajax({
			type: "GET",
			url: servicePath + "/dictData/delete?charset=utf-8&dictDataId=" + dictDataId + "&loginCode=" + adminInfo.loginCode,
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			success: function(data) {
				layer.close(layerIndex);
