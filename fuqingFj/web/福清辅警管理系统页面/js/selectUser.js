			...
			success: function(data) {
				if (data.errCode == 0) {
					tree.render({
						...
						click: function(obj) {
							$(".layui-tree-txt").css("color","#555");//选中样式#009688、#39b90f
							$(".layui-tree-txt").css("fontWeight","");//去掉加粗
							var elem=obj.elem[0].firstElementChild.firstElementChild.children[1];
							...
							if(formData.onlyCurrnetOrg=="true"){
								whereDate={"orgCode": obj.data.orgCode,"amUnitId": null};
							}else{
								whereDate={"orgCode": null,"amUnitId": obj.data.id};
							}
							ryTable.reload({
								where: whereDate
							});
